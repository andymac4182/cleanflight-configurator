(function() {
	...
	function cleanflightConfigurator($log, $serialService) {
		var service = {
			...
			check: check,
			update_port_select: update_port_select,
			port_detected: port_detected,
			port_removed: port_removed,
			array_difference: array_difference,
			flush_callbacks: flush_callbacks
		};
		...
		function check() {
			...
			$serialService.getDevices(function(current_ports) {
				...
				if (self.array_difference(self.initial_ports, current_ports).length >
					0 ||
					!self.initial_ports) {
					var removed_ports = self.array_difference(self.initial_ports,
						current_ports);

					if (self.initial_ports != false) {
						if (removed_ports.length > 1) {
							$log.log('PortHandler - Removed: ' + removed_ports);
						} else {
							$log.log('PortHandler - Removed: ' + removed_ports[0]);
						}
					}
							...
							if (result.last_used_port) {
								current_ports.forEach(function(port) {
									if (port == result.last_used_port) {
										$log.log('Selecting last used port: ' + result.last_used_port);
					...
					if (!self.initial_ports) {
						...
						for (var i = 0; i < removed_ports.length; i++) {
							self.initial_ports.splice(self.initial_ports.indexOf(removed_ports[
									i]),
								1);
						}
					}
				...
				if (new_ports.length) {
					if (new_ports.length > 1) {
						$log.log('PortHandler - Found: ' + new_ports);
					} else {
						$log.log('PortHandler - Found: ' + new_ports[0]);
					}
			...
			function update_port_select(ports) {
			...
			function port_detected(name, code, timeout, ignore_timeout) {
				var self = this;
				var obj = {
					'name': name,
					'code': code,
				...
				if (!ignore_timeout) {
					obj.timer = setTimeout(function() {
						$log.log('PortHandler - timeout - ' + obj.name);
			...
			function port_removed(name, code, timeout, ignore_timeout) {
				var self = this;
				var obj = {
					'name': name,
					'code': code,
			...
			function flush_callbacks() {
				var killed = 0;

				for (var i = this.port_detected_callbacks.length - 1; i >= 0; i--) {
					if (this.port_detected_callbacks[i].timer) clearTimeout(this.port_detected_callbacks[
						i].timer);
				...
				for (var i = this.port_removed_callbacks.length - 1; i >= 0; i--) {
					if (this.port_removed_callbacks[i].timer) clearTimeout(this.port_removed_callbacks[
						i].timer);
