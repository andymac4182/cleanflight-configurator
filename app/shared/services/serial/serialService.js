(function() {
	...
	function serialService($log) {
		var service = {
			...
			emptyOutputBuffer: emptyOutputBuffer,
		...
		function connect(path, options, callback) {
			var self = this;
			...
			chrome.serial.connect(path, options, function(connectionInfo) {
				...
				if (connectionInfo && !self.openCanceled) {
					...
					self.onReceiveError.addListener(function watch_for_on_receive_errors(
						info) {
						$log.error(info);
									...
									chrome.serial.setPaused(self.connectionId, false, function() {
										self.getInfo(function(info) {
											if (info) {
												if (!info.paused) {
													$log.log(
														'SERIAL: Connection recovered from last onReceiveError'
													);
													googleAnalytics.sendException(
														'Serial: onReceiveError - recovered', false);
													...
													GUI.log(
														'Unrecoverable <span style="color: red">failure</span> of serial connection, disconnecting...'
													);
													googleAnalytics.sendException(
														'Serial: onReceiveError - unrecoverable', false);
					...
					$log.log('SERIAL: Connection opened with ID: ' + connectionInfo.connectionId +
						', Baud: ' + connectionInfo.bitrate);
					...
					self.openRequested = false;
					$log.log('SERIAL: Failed to open serial port');
		...
		function disconnect(callback) {
			...
			if (self.connectionId) {
				...
				chrome.serial.disconnect(this.connectionId, function(result) {
					...
					if (result) {
						$log.log('SERIAL: Connection with ID: ' + self.connectionId +
							' closed, Sent: ' + self.bytesSent + ' bytes, Received: ' + self.bytesReceived +
							' bytes');
					} else {
						$log.log('SERIAL: Failed to close connection with ID: ' + self.connectionId +
							' closed, Sent: ' + self.bytesSent + ' bytes, Received: ' + self.bytesReceived +
							' bytes');
		...
		function getControlSignals(callback) {
			chrome.serial.getControlSignals(this.connectionId, callback);
		};

		function setControlSignals(signals, callback) {
				...
				chrome.serial.send(self.connectionId, data, function(sendInfo) {
					...
					if (self.outputBuffer.length) {
						...
						if (self.outputBuffer.length > 100) {
							...
							$log.log('SERIAL: Send buffer overflowing, dropped: ' + counter +
								' entries');
						}
